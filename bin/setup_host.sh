...
echo 'docker run ...'
docker run -e RAILS_MASTER_KEY=$RAILS_MASTER_KEY -e DB_HOST=$DB_HOST -d -p 3000:3000 --network=network1 -e DB_PASSWORD=$DB_PASSWORD --name=$container_name mangosteen:$version
echo 'DONE!'